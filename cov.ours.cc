...
#if __has_include("src/sandbox/hardware-support.h")
#include "src/sandbox/hardware-support.h"
#endif

// SHM_SIZE and TYPE_COV_* constants are defined in cov.h.
// MAX_EDGES is capped so the code coverage bitmap cannot overrun
// into the type coverage region at the end of shared memory.
...
void record_type(uint16_t location_id, uint16_t type_id) {
  ...
  uint8_t* type_bits = reinterpret_cast<uint8_t*>(shmem) + TYPE_COV_OFFSET;
  uint32_t idx =
